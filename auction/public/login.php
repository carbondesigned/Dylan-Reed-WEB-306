...
if(isset($_POST['submit'])) {
	...
	if($user->get('active')) {
		...
		// (B) REGISTER THE USER OBJECT TO THE SESSION OBJECT USING THE METHOD IN THE SESSION CLASS BELOW
            // register the user object to the session
	...
	if(isset($_GET['error'])) {
		try {
            ...
            if (isset($_GET['error'])) {
                try {
                    // retrieve the error message using the correct method in the user class
                    $errorMsg = User::$errorArray[$_GET['error']];
                } catch (ClassException $e) {
                    Logger::getLogger()->error("invalid error code", ['error' => $e]);
