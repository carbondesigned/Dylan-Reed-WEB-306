...
if(isset($_POST['submit'])) {
	...
	if(!$file->get("name")) {
		...
		try {
            ...
            $image = new Image($item->get('id'), $file->get('name'), $file->get('size'), $file->get('type'), $file->get('path'));
	...
	if(isset($_GET['error'])) {
		...
		<form enctype="multipart/form-data" action="<?php echo $_SERVER['REQUEST_URI']; ?>"
			...
			<!-- Input 1 -->
            <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo File::MAXFILESIZE; ?>"/>
