...
if (isset($_POST['submit'])) {
	...
	$comment = new Comment(0, $validId, 0, $POST_NAME['name'], $_POST['comment']);
	$comment->create();
	...
	if (count($comments) == 0) {
		...
		echo "<p>(<strong>" . count($comments) . "</strong>) comments:</p>";
		$i = 1;
		foreach ($comments as $comment) {
			echo "<div class='comment'>";
			echo "<p class='commentheader'>" . $comment->name . " - " . date("D jS F Y g.iA", strtotime($comment->date)) . "</p>";
			echo "<p class='commentbody'>" . nl2br($comment->comment) . "</p>";
			echo "</div>";
			$i++;
		}
	}
